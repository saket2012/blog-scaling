...
def post_article(user_id, text, author, title):
    conn = get_db()
    c = conn.cursor()
    unix = int(time.time())
    ...
    try:
        c.execute("""INSERT INTO articles VALUES (NULL, ?, ?, ?, ?, ?, ?)""", (
            user_id, text, author, title, post_time, last_updated_time))
        conn.commit()
    except Exception:
        conn.rollback()
...
def get_n_articles(n):
    ...
    c = conn.cursor()
    c.execute(
        """SELECT text, author, title, post_time, post_time FROM articles ORDER BY \
...
def get_articles_metadata(n):
    ...
    c = conn.cursor()
    c.execute(
        """SELECT text, author, title  FROM articles ORDER BY post_time LIMIT ?""", (n,))
    row_headers = [x[0] for x in c.description]
    rows = c.fetchall()
    articles = []
    for article in rows:
